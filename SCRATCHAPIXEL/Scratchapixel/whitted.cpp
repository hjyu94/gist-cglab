...
// ax^2 + bx + c = 0 방정식의 해가 있으면 true, 없으면 false를 리턴한다
// 해가 있다면 x0, x1에 저장한다. (x0<x1)
...
public:
	...
	{
		// analytic solution
		Vec3f L = orig - center; // normal vector?
	...
	// orig에서 dir 방향으로 향하는 ray가 있다
	// Sphere와 그 ray가 교점이 있으면 true, 없으면 false를 리턴한다.
	// t0, t1은 교점과의 거리를 나타낸다.

	void getSurfaceProperties(const Vec3f &P, const Vec3f &I, const uint32_t &index, const Vec2f &uv, Vec3f &N, Vec2f &st) const
	{
		N = normalize(P - center);
	}
	// Center에서 P로 가는 벡터의 normalized 벡터를 출력한다.
...
// hitObject가 null이 아니면, 즉 obj가 부딪혔으면 true 리턴
// 부딪히지 않았으면 false 리턴
...
// we mix the reflection and refraction color using the result of the fresnel equations 
// (it computes the amount of reflection and refractin depending on the surface normal, 
